...
public sealed class EntityNotRequirementTests : IDisposable
{
    ...
    [InlineData(true, 1, 0)]
    [InlineData(true, 2, 0)]
    public void IsMet_ShouldReturnExpected(bool expected, int current, int excluded)
    {
        var subject = new EntityNotRequirement(_entity, excluded);
    ...
    {
        var subject = new EntityNotRequirement(_entity, 1);
